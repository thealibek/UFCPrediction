...
def llm_predict_v2(fa, fb, ctx):
    ...
    user_msg = (
        ...
        f"=== БОЕЦ A ===\n{fa.get('name')} | record: {fa.get('record')} | country: {fa.get('country')}\n"
        f"=== БОЕЦ B ===\n{fb.get('name')} | record: {fb.get('record')} | country: {fb.get('country')}\n\n"
        f"{lessons_block}\n\n"
...
def main():
    ...
    path = bt.run_blind_test(
        event_name=target["name"] + " [v2-lessons]",
        event_date=target["date"],
        fights=blinded, predict_fn=predict_with_venue,
        model_meta={"llm_model": LLM_MODEL, "system_prompt_version": "blind-v2-lessons"},
