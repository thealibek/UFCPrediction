...
Сохраняет под -v5-expanded-lessons суффиксом чтобы не перезатереть V4.
"""
...
def already_done_v5(event_id):
    for p in Path("blind_tests").glob("*-v5-*.json"):
        try:
...
def main():
    print(f"📡 V5 mass blind 2026 | model={LLM_MODEL} | 17 lessons")
    ...
    for i, ev in enumerate(to_run, 1):
        ...
        try:
            path = bt.run_blind_test(
                event_name=ev["name"] + " [v5-expanded-lessons]",
                event_date=ev["date"],
                fights=blinded, predict_fn=predict_with_venue,
                model_meta={"llm_model": LLM_MODEL,
                            "system_prompt_version": "blind-v5-expanded-lessons",
    ...
    rows = []
    for p in sorted(Path("blind_tests").glob("*-v5-*.json")):
        d = json.loads(p.read_text())
        s = d.get("summary",{})
